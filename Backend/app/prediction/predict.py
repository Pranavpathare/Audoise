...
def predict_one(path: str):
    ...
    X = np.expand_dims(X, -1) # Reshape to 1D
    
    prediction = model.predict(X)
    prediction = np.argmax(prediction, axis = 1)[0] # Max Index is Prediction
...
def predict_many(path: str):
    ...
    split_time = max(5, int(wav_sound.duration_seconds) // 15)
    ...
    for i, chunk in enumerate(audio_chunks):
        ...
        file_path = "prediction/_temp_audio_files_/chunk{}.wav".format(i + 1)
